...
int findSiblingID(int nodeID)
{
    if (nodeID == 0)
        return -1; // 根节点没有兄弟节点
    if (nodeID % 2 == 0)
        return nodeID - 1; // 偶数节点的左兄弟节点ID
    else
        return nodeID + 1; // 奇数节点的右兄弟节点ID
}
// 根据节点ID查找左孩子节点ID
...
void leaveTree(int nodeID, BIGNUM *key)
{
    ...
    // sponsor移动后的ID
    int sponsorID = findLowestRightID(parentID);
...
void nodeLeaveTree(int nodeID)
{
    ...
    moveSubTree(siblingID, parentID);
    // 更新自己信息 自己在子树中，则需要更新自己的ID；否则不需要更新 //new delete
