...
void parsePacket0(char *buffer)
{
    ...
    if (header->type != 0)
    {
        printf("Error: parseCreatePacket failed.\n");
        return;
    }
    int index = sizeof(DataHeader);
    // groupname 32bytes
...
void parsePacket1(char *buffer)
{
    ...
    if (header->type != 1)
    {
        printf("Error: parseJoinPacket failed.\n");
        return;
    }
    int index = sizeof(DataHeader);
...
int parsePacket2(char *buffer)
{
    ...
    if (header->type != 2)
    {
        printf("Error: parseLeavePacket failed.\n");
        return -1;
    }
    int index = sizeof(DataHeader);
    // groupname 32bytes
...
void handlePacket(Packet *packet)
{
    ...
    else if (packet->header.type == 1)
    {
        ...
        fprintf_log(log_file, "receive udp(type:1): as the sponsor, receive a leave quest\n");
    }
    else if (packet->header.type == 2)
    {
        ...
        broadcast2leaf(packet);
        fprintf_log(log_file, "receive udp(type:2): as the sponsor, receive a leave quest; leave_node_id:%d\n", leave_node_id);
    }
...
void send2node(Packet *packet, int nodeID)
{
    ...
    fprintf_log(log_file, "udp:%d send to %d, type: %u, tx_pkt_num:%d\n", key_self->id, nodeID, packet->header.type, tx_pkt_num);
    tx_pkt_num++;
    // 关闭socket
    close(sockfd);
}
